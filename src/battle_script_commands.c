...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        else if ((gBattleWeather & B_WEATHER_HAIL) && (move == MOVE_BLIZZARD || move == MOVE_HURRICANE))
        {
            // thunder/hurricane ignore acc checks in rain unless target is holding utility umbrella
            JumpIfMoveFailed(7, move);
            return TRUE;
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            calculatedExp = gBaseStats[gBattleMons[gBattlerFainted].species].expYield * gBattleMons[gBattlerFainted].level * 3 / 7;
        ...
        case ITEM_MOON_BALL:
            for (i = 0; i < EVOS_PER_MON; i++)
            {
                if (gEvolutionTable[gBattleMons[gBattlerTarget].species][i].method == EVO_ITEM
                    ...
                    ballMultiplier = 40;
                else if (gBattleMons[gBattlerTarget].species == SPECIES_NIDORAN_F || gBattleMons[gBattlerTarget].species == SPECIES_NIDORAN_M)
                    ballMultiplier = 40;
            }
            break;
        case ITEM_LOVE_BALL:
